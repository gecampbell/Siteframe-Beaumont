...
{* Trackbacks *}
{if $trackback_enable && $num_trackbacks}
<div class="comments">
<h2>Trackbacks</h2>
...
<div class="comment">
  <div class="comment_subject">
    {if $_editor}
    <div class="comment_buttons">
    <a href="{$trackback.delete_url}">Delete</a>
    </div>
    {/if}
  {$trackback.tb_title|default:"Trackback"}</div>
  <div class="comment_text">{$trackback.tb_excerpt|strip_tags}</div>
  <div class="comment_info">Trackback from
  <a href="{$trackback.tb_url}">{$trackback.tb_blog_name}</a>
  </div>
</div>
{/all}
</div>
...
{if $cuser.user_id && $allow_comments}
    {menu name="action"
          link = "#comment"
          onclick="javascript:show('cform');focus('commentform');"
          text="New Comment"}
