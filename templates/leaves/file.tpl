{* $Id: file.tpl,v 1.3 2005/12/22 03:06:12 glen Exp $ *}
...
{if $file_mime_type eq "image"}
<a href="javascript:popup('{$site_path}/file.php?id={$file_id}&amp;template=imagepopup','image','height={$image_y},width={$image_x}');"  title="Click to view full-sized" ><img src="{thumbnail file=$file_path type=$file_type maxw=450}" alt="{$file_name}" height="{$im_h}" width="{$im_w}" /></a>
{else}
